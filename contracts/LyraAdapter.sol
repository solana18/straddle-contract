...
contract LyraAdapterNew is Ownable {
    ...
    IOptionMarketNew public optionMarket;
   ...
   */
    ...
    function setLyraAddresses(
        ...
        // Do approvals
        IDelegateApprovals(synthetixAdapter.delegateApprovals())
            .approveExchangeOnBehalf(address(synthetixAdapter));
    ...
    function _openPosition(
        ...
        IOptionMarketNew.Result memory result = optionMarket.openPosition(
    ...
    function _closeOrForceClosePosition(
        ...
        if (
            !_isOutsideDeltaCutoff(params.strikeId) &&
            !_isWithinTradingCutoff(params.strikeId)
        ) {
    ...
    function _closePosition(
        ...
        IOptionMarketNew.Result memory result = optionMarket.closePosition(
            _convertParams(params)
        );
        if (params.rewardRecipient != address(0)) {
            feeCounter.trackFee(
    ...
    function _forceClosePosition(
        ...
        IOptionMarketNew.Result memory result = optionMarket.forceClosePosition(
            _convertParams(params)
        );
        if (params.rewardRecipient != address(0)) {
            feeCounter.trackFee(
    ...
    function _exchangeFromExactQuote(
        uint amountQuote,
        uint minBaseReceived
    ) internal returns (uint baseReceived) {
        baseReceived = synthetixAdapter.exchangeFromExactQuote(
            address(optionMarket),
            amountQuote
        );
        if (baseReceived < minBaseReceived) {
            revert ExchangerBaseReceivedTooLow(
                address(this),
                minBaseReceived,
                baseReceived
            );
    ...
    function _exchangeToExactQuote(
        uint amountQuote,
        uint maxBaseUsed
    ) internal returns (uint quoteReceived) {
        ISynthetixAdapter.ExchangeParams
            memory exchangeParams = synthetixAdapter.getExchangeParams(
                address(optionMarket)
            );
    ...
    function _exchangeFromExactBase(
        uint amountBase,
        uint minQuoteReceived
    ) internal returns (uint quoteReceived) {
        quoteReceived = synthetixAdapter.exchangeFromExactBase(
            address(optionMarket),
            amountBase
        );
        if (quoteReceived < minQuoteReceived) {
            revert ExchangerQuoteReceivedTooLow(
                address(this),
                minQuoteReceived,
                quoteReceived
            );
    ...
    function _getExchangeParams()
        internal
        view
        returns (ExchangeRateParams memory)
    {
        ISynthetixAdapter.ExchangeParams memory params = synthetixAdapter
    ...
    ) internal view returns (OptionPosition[] memory) {
        IOptionToken.OptionPosition[] memory positions = optionToken
    ...
    function _splitPosition(
        ...
        newPositionId = optionToken.split(
            positionId,
            newAmount,
            newCollateral,
            recipient
    ...
    function _getBoard(uint boardId) internal view returns (Board memory) {
        IOptionMarketNew.OptionBoard memory board = optionMarket.getOptionBoard(
            boardId
        );
        return
            Board({
    ...
    ) internal view returns (Strike[] memory allStrikes) {
        uint strikesLen = strikeIds.length;

        allStrikes = new Strike[](strikesLen);
        for (uint i = 0; i < strikesLen; ++i) {
            (
                IOptionMarketNew.Strike memory strike,
                IOptionMarketNew.OptionBoard memory board
    ...
    function _getVols(
        uint[] memory strikeIds
    ) internal view returns (uint[] memory vols) {
        uint strikesLen = strikeIds.length;
    ...
    function _getDeltas(
        uint[] memory strikeIds
    ) internal view returns (int[] memory callDeltas) {
        uint strikesLen = strikeIds.length;
    ...
    function _getVegas(
        uint[] memory strikeIds
    ) internal view returns (uint[] memory vegas) {
        uint strikesLen = strikeIds.length;
    ...
    function _getPurePremium(
        ...
        BlackScholes.BlackScholesInputs memory bsInput = BlackScholes
            .BlackScholesInputs({
                ...
                rateDecimal: synthetixAdapter.rateAndCarry(
                    address(optionMarket)
                )
            });
    ...
    function _getPurePremiumForStrike(
        uint strikeId
    ) internal view returns (uint call, uint put) {
    ...
    function _getLiquidity() internal view returns (Liquidity memory) {
        ILiquidityPool.Liquidity memory liquidity = liquidityPool
            .getLiquidity();
        return
            Liquidity({
    ...
    function _getMarketParams() internal view returns (MarketParams memory) {
        IOptionMarketPricer.PricingParameters
            memory pricingParams = optionPricer.getPricingParams();
        IOptionMarketPricer.TradeLimitParameters
            memory tradeLimitParams = optionPricer.getTradeLimitParams();
        return
            MarketParams({
                standardSize: pricingParams.standardSize,
                skewAdjustmentParam: pricingParams.skewAdjustmentFactor,
                rateAndCarry: synthetixAdapter.rateAndCarry(
                    address(optionMarket)
                ),
    ...
    function _isWithinTradingCutoff(
        uint strikeId
    ...
    function _getMinCollateral(
        ...
        return
            greekCache.getMinCollateral(
                IOptionMarket.OptionType(uint(optionType)),
                strikePrice,
                expiry,
                spotPrice,
                amount
    ...
    function _getMinCollateralForPosition(
        uint positionId
    ) internal view returns (uint) {
        IOptionToken.PositionWithOwner memory position = optionToken
    ...
    function _deltaGWAV(
        uint strikeId,
        uint secondsAgo
    ) internal view returns (int callDelta) {
    ...
    function _optionPriceGWAV(
        uint strikeId,
        uint secondsAgo
    ) internal view returns (uint callPrice, uint putPrice) {
    ...
    function _getBsInput(
        ...
        bsInput = BlackScholes.BlackScholesInputs({
            ...
            spotDecimal: synthetixAdapter.getSpotPriceForMarket(
                address(optionMarket)
            ),
    ...
    ) internal pure returns (IOptionMarketNew.TradeInputParameters memory) {
        return
            IOptionMarketNew.TradeInputParameters({
                strikeId: _params.strikeId,
                positionId: _params.positionId,
                iterations: _params.iterations,
                optionType: IOptionMarketNew.OptionType(uint(_params.optionType)),
                amount: _params.amount,
                setCollateralTo: _params.setCollateralTo,
                minTotalCost: _params.minTotalCost,
                maxTotalCost: _params.maxTotalCost,
                rewardRecipient: _params.rewardRecipient
            });
    }

    /// @dev get lyra market addresses from LyraRegistry
    function _assignLyraRegistryMarketAddresses() internal {
        ILyraRegistry.OptionMarketAddresses memory addresses = lyraRegistry
            .getMarketAddresses(address(optionMarket));
    ...
    error ExchangerBaseReceivedTooLow(
        address thrower,
        uint baseExpected,
        uint baseReceived
    );
    error ExchangerQuoteReceivedTooLow(
        address thrower,
        uint quoteExpected,
        uint quoteReceived
    );
