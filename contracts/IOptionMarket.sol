...
import "@lyrafinance/protocol/contracts/interfaces/IOptionMarketPricer.sol";

// For full documentation refer to @lyrafinance/protocol/contracts/OptionMarket.sol";
interface IOptionMarketNew {
    enum TradeDirection {
        OPEN,
        CLOSE,
        LIQUIDATE
    ...
    function getOptionMarketParams()
        external
        view
        returns (OptionMarketParameters memory);
    ...
    function getStrikeAndExpiry(
        uint strikeId
    ) external view returns (uint strikePrice, uint expiry);

    function getBoardStrikes(
        uint boardId
    ) external view returns (uint[] memory strikeIds);
    ...
    function getStrikeAndBoard(
        uint strikeId
    ) external view returns (Strike memory, OptionBoard memory);
    ...
    function getBoardAndStrikeDetails(
        ...
        external
        view
        returns (OptionBoard memory, Strike[] memory, uint[] memory, uint);
    ...
    // Admin
    error InvalidOptionMarketParams(
        address thrower,
        OptionMarketParameters optionMarketParams
    );
